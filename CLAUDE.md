...
- **Perfekte Woche**: Bonus (15 💎), wenn alle 7 Tage der aktuellen App-Woche `minutes >= 60` (mind. 1 Stunde) in `study_days` haben — freie Tage retten nicht (gleiche Designentscheidung wie bei Streaks). Immer sichtbare Zeile `#perfect-week-row` oberhalb der Tier-Tabs in `#challenges-card` (`renderPerfectWeekRow()`, tier-unabhängig), wird zusätzlich aus `creditFocusMinutes()` neu gerendert (kein Fetch), damit der Fortschritt sofort nach jeder Session aktuell ist. `claim_perfect_week()` verlangt serverseitig zusätzlich `today >= week_start + 6` (Woche muss vorbei sein) als Verteidigung in der Tiefe, da `study_days`-Schreibpfade das Datum nicht gegen „heute" validieren (siehe `add_study_minutes` unten). Die 60-Minuten-Schwelle ist serverseitig hardcodiert (`20260725000000_perfect_week_min_60min.sql`), kein Konfigurationswert wie bei Streak-Meilensteinen.
