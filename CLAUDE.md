...
- **DOM**: 3 neue IDs (`#timer-card`, `#heatmap-card`, `#stats-card`) plus 3 neue Wrapper-Divs `.bento-row.bento-row-main` (Heatmap+Stats), `.bento-row.bento-row-secondary` (Challenges-Card + `#egg-section-wrapper`, inkl. der dazwischenliegenden `position:fixed`-Overlays wie `#egg-placeholder-overlay` — deren Position im Baum ist irrelevant, `position:fixed` wird nie zum Flex-Item), `.bento-row.bento-row-leaderboard` (Leaderboard allein).
