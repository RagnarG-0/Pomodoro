...
- **Histogramm-Darstellung**: X-Achse = Tageszeit 04:00 (links) → 04:00 (rechts), normale Leserichtung (bewusst nicht invertiert wie die Y-Achse des Punktdiagramms darüber — Balken stehen, Zeit gehört auf die X-Achse), gleiche Stunden-Gitterlinien-Konvention (`04/08/12/16/20/00/04`) wie oben für eine konsistente Zeit-Referenz zwischen beiden Karten. Y-Achse = relative Dichte, auf den höchsten Balken normiert (0–100 %, eine rohe KDE-Dichte-Einheit wäre nicht aussagekräftig). `ATTENTION_HISTOGRAM_BIN_MIN`-Bins (Default 30 min), Balkenhöhe = KDE-Dichte an der Bin-Mitte (gleiche Bandbreite wie die Peak-Erkennung). Balken innerhalb eines erkannten Risiko-Intervalls: `fill="#dc2626"` voll deckend; außerhalb: `fill-opacity="0.22"` (bewusst statt `color-mix()` — breiterer Browser-Support in SVG-Präsentationsattributen). DBSCAN-Cluster als eigene Marker-Reihe unterhalb der Stunden-Beschriftung (`#64b5f6`, bereits als „Info"-Farbe etabliert, siehe Leaderboard-Rang-Indikator „Neu im Leaderboard"), Ausreißer als kleine graue Punkte; ein wrap-Cluster wird als zwei Liniensegmente bis zum jeweiligen Rand gezeichnet.
